...
function saveRelease({ version, file = "", installerFile = "", notes = "", forceUpdate = true }) {
  ...
  writeJson(UPDATE_JSON_PATH, {
    ...
    installerUrl: installerFile ? `/download/${installerFile}` : "",
    forceUpdate: Boolean(forceUpdate),
    changelog: notes
  });
...
function serveLocalFile(res, filePath, contentType) {
...
function resolveBrandIconFile() {
  const candidates = [
    path.join(__dirname, "..", "renderer-v2", "assets", "baiqiu-icon.png"),
    path.join(__dirname, "..", "renderer-v2", "assets", "baiqiu-icon.ico"),
    path.join(__dirname, "assets", "icon.ico"),
    path.join(__dirname, "..", "assets", "icon.ico"),
    path.join(__dirname, "..", "assets", "branding", "web", "icon-192.png")
  ];
...
<body>
  <main>
    ...
    <div class="bar">
      ...
      <label>允许设备数
        <input id="maxDevices" type="number" min="3" max="3" value="3" readonly>
      </label>
      <label>到期时间
    ...
    async function createLicenses() {
      try {
        const body = {
          ...
          maxDevices: Number(document.getElementById("maxDevices").value || 3),
...
function downloadPage(origin) {
  ...
  const installerUrl = release && isValidWindowsInstaller(release) && release.installerFile ? `${origin}/download/${encodeURIComponent(release.installerFile)}` : "";
...
function downloadLandingPage(origin) {
  const release = latestRelease("stable");
  const brandIcon = resolveBrandIconFile();
  const brandIconUrl = brandIcon ? `${origin}/brand-icon` : "";
  const installerUrl = release && isValidWindowsInstaller(release) && release.installerFile ? `${origin}/download/${encodeURIComponent(release.installerFile)}` : "";
  const updateZipUrl = release && hasReleaseFile(release, "zip") ? `${origin}/baiqiu-${encodeURIComponent(release.version)}.zip` : "";
  const primaryUrl = installerUrl || updateZipUrl;
  const primaryLabel = installerUrl ? "一键安装版下载" : "下载压缩包";
  const hasFallback = Boolean(installerUrl && updateZipUrl);
  const releaseTime = release?.publishedAt ? new Date(release.publishedAt).toLocaleString("zh-CN", { hour12: false }) : "";
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  ...
  <link rel="icon" href="${brandIconUrl || "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='16' fill='%232563eb'/%3E%3Cpath d='M20 46V18h12c7 0 12 3 12 9 0 4-2 7-6 8 5 1 8 4 8 9 0 6-5 10-13 10H20Zm10-16c4 0 6-1 6-4s-2-4-6-4h-3v8h3Zm1 11c5 0 7-1 7-4s-2-4-7-4h-4v8h4Z' fill='white'/%3E%3C/svg%3E"}">
  <style>
    :root {
      color-scheme: light;
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 640px) {
      ...
      .hero-main, .hero-side, .section { padding: 18px; border-radius: 22px; }
  ...
  <main>
        ...
        <div>
          <div class="brand">
            <div class="brand-mark">
              ${brandIconUrl ? `<img src="${brandIconUrl}" alt="白球 AI 桌面图标">` : `<span style="font-weight:900;color:#2563eb;font-size:20px;">白球</span>`}
            </div>
            <div>
              <span class="eyebrow">白色高级下载页 · 0基础可安装</span>
              <h1>白球 AI 客户端</h1>
            </div>
          </div>
          <p class="lead">这是一页给客户直接下载、直接安装的白色新版页面：界面更干净，下载更明确，默认优先提供一键安装版；如果你电脑比较旧，也保留压缩包备用。</p>
          <div class="actions">
            ${primaryUrl ? `<a class="button primary" href="${primaryUrl}">${primaryLabel}</a>` : `<span class="button primary" style="pointer-events:none;opacity:.6;">暂时没有可下载版本</span>`}
            ${hasFallback ? `<a class="button secondary" href="${updateZipUrl}">备用压缩包下载</a>` : ""}
          </div>
        </div>
        <div class="stats">
          <div class="stat"><strong>一键安装</strong><span>双击安装包即可完成，适合大多数 Windows 电脑。</span></div>
          <div class="stat"><strong>零基础友好</strong><span>步骤更少，桌面图标更醒目，客户更容易自己装。</span></div>
          <div class="stat"><strong>白色高级感</strong><span>浅色背景、留白、轻阴影，整体更像正式官网。</span></div>
        </div>
      </section>
      <aside class="card hero-side">
        <div class="side-block">
          <h2 class="side-title">快速下载</h2>
          <p class="side-text">右侧留空区直接放下载入口，不让客户在页面上找来找去。</p>
          <div class="download-list">
            <div class="download-item">
              <strong>${primaryLabel}</strong>
              <span>当前推荐版本：${release ? `v${release.version}` : "暂无"}</span>
            </div>
            ${updateZipUrl ? `<div class="download-item"><strong>压缩包备用</strong><span>适合老电脑、便携使用或安装器被拦截时切换。</span></div>` : ""}
          </div>
        </div>
        <div class="side-block">
          <h2 class="side-title">安装说明</h2>
          <ul class="side-text" style="margin:0;padding-left:18px;">
            <li>下载后直接打开安装包。</li>
            <li>首次启动按提示授权即可。</li>
            <li>安装完成后桌面会保留白球 AI 图标。</li>
          </ul>
        </div>
      </aside>
    </div>
    <div class="section-grid">
      <section class="card section">
        <h2>适配说明</h2>
        <ul>
          <li>优先适配 Windows 10 / 11 64 位。</li>
          <li>如果安装器无法运行，可改用压缩包。</li>
          <li>页面内下载按钮直接指向当前版本。</li>
        </ul>
      </section>
      <section class="card section">
        <h2>客户体验</h2>
        <ul>
          <li>首页先看见图标，再看见下载按钮。</li>
          <li>文案更短，操作路径更直。</li>
          <li>页面留白更多，观感更稳。</li>
        </ul>
      </section>
      <section class="card section">
        <h2>发布信息</h2>
        <ul>
          <li>当前版本：${release ? `v${release.version}` : "暂无"}</li>
          <li>发布时间：${releaseTime || "暂无"}</li>
          <li>后台地址：<code>/admin</code></li>
        </ul>
      </section>
    </div>
    <div class="footer">如果你要我继续，我可以把这个页面再压得更像“正式商用官网”，或者直接帮你把服务器端发布链路也捋顺。</div>
  </main>
</body>
</html>`;
}
...
function createLicenses(count = 1, expiresAt = "2099-12-31T23:59:59Z", maxDevices = 3, notes = "") {
  const db = readDb();
  db.licenses ||= [];
...
function membershipUserId(item) {
  if (item.userId) return item.userId;
  item.userId = `user-${crypto.createHash("sha256").update(`${item.code}:${item.createdAt || Date.now()}`).digest("hex").slice(0, 20)}`;
  return item.userId;
}

function publicMembership(item, deviceId) {
  const expiresAt = item.expiresAt || "2099-12-31T23:59:59Z";
  const devices = Array.isArray(item.devices) ? item.devices : [];
  return {
    userId: membershipUserId(item),
    membershipLevel: item.plan || (String(expiresAt).startsWith("2099-") ? "lifetime" : "member"),
    plan: item.plan || (String(expiresAt).startsWith("2099-") ? "lifetime" : "member"),
    planName: item.planName || (String(expiresAt).startsWith("2099-") ? "永久会员" : "限时会员"),
    lifetime: Boolean(item.lifetime || String(expiresAt).startsWith("2099-")),
    expireTime: expiresAt,
    activationCode: item.code,
    deviceBinding: deviceId,
    deviceBindings: devices.map((device) => ({ ...device })),
    activationHistory: Array.isArray(item.activationHistory) ? item.activationHistory.slice(-50) : [],
    createdAt: item.createdAt || Date.now(),
    customer: item.customer || {},
    signature: serverSignature(item.code, deviceId, expiresAt)
  };
}
...
async function handleActivate(req, res) {
  ...
  const deviceLimit = 3;
  if (!existingDevice && item.devices.length >= deviceLimit) {
    return sendJson(res, 403, { success: false, message: "该兑换码最多绑定 3 台电脑，当前设备无法激活。" });
  }

  if (!existingDevice) {
    item.devices.push({ deviceId, sourceIp, activatedAt: Date.now(), lastSeenAt: Date.now() });
    item.status = "used";
  } else {
    existingDevice.lastSeenAt = Date.now();
    existingDevice.sourceIp = sourceIp || existingDevice.sourceIp || "";
  }
  item.customer = {
    name: String(body.name || body.customer || item.customer?.name || "").trim(),
    phone: String(body.phone || item.customer?.phone || "").trim()
  };
  item.plan = String(item.plan || body.plan || "");
  item.planName = String(item.planName || body.planName || "");
  item.lifetime = Boolean(item.lifetime || body.lifetime || String(item.expiresAt || "").startsWith("2099-"));
  membershipUserId(item);
  item.activationHistory ||= [];
  item.activationHistory.push({ deviceId, sourceIp, activatedAt: Date.now() });
  item.activationHistory = item.activationHistory.slice(-50);
  item.updatedAt = Date.now();
  writeDb(db);

  const expiresAt = item.expiresAt || "2099-12-31T23:59:59Z";
  const membership = publicMembership(item, deviceId);
  return sendJson(res, 200, {
    ...
    activationHistory: membership.activationHistory,
    license: membership,
    message: "激活成功"
  });
}

function handleMembershipStatus(req, res, url) {
  const deviceId = String(url.searchParams.get("deviceId") || "").trim();
  if (!deviceId) return sendJson(res, 400, { success: false, message: "缺少设备 ID" });
  const db = readDb();
  const item = (db.licenses || []).find((entry) => (entry.devices || []).some((device) => device.deviceId === deviceId));
  if (!item) return sendJson(res, 404, { success: false, message: "当前设备没有可恢复的会员状态" });
  if (item.status === "banned") return sendJson(res, 403, { success: false, message: "会员授权已停用" });
  if (item.expiresAt && Date.parse(item.expiresAt) < Date.now()) return sendJson(res, 403, { success: false, message: "会员已过期" });
  const binding = item.devices.find((device) => device.deviceId === deviceId);
  binding.lastSeenAt = Date.now();
  item.updatedAt = Date.now();
  writeDb(db);
  return sendJson(res, 200, { success: true, membership: publicMembership(item, deviceId) });
}

async function handleMembershipDeviceUnbind(req, res) {
  const body = await readBody(req);
  const code = normalizeCode(body.code || body.activationCode);
  const currentDeviceId = String(body.currentDeviceId || "").trim();
  const targetDeviceId = String(body.targetDeviceId || "").trim();
  if (!isCodeFormatValid(code) || !currentDeviceId || !targetDeviceId) {
    return sendJson(res, 400, { success: false, message: "缺少有效的兑换码或设备 ID" });
  }
  const db = readDb();
  const item = (db.licenses || []).find((entry) => entry.code === code);
  if (!item || !(item.devices || []).some((device) => device.deviceId === currentDeviceId)) {
    return sendJson(res, 403, { success: false, message: "当前设备无权管理该会员" });
  }
  const before = item.devices.length;
  item.devices = item.devices.filter((device) => device.deviceId !== targetDeviceId);
  if (item.devices.length === before) return sendJson(res, 404, { success: false, message: "目标设备不存在" });
  item.updatedAt = Date.now();
  writeDb(db);
  return sendJson(res, 200, { success: true, message: "设备授权已解除", deviceBindings: item.devices });
}
    ...
    .map((item) => {
      ...
      return {
        ...
        downloadUrl: item.installerFile
          ? `${origin}/download/${encodeURIComponent(item.installerFile)}`
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && (url.pathname === "/brand-icon" || url.pathname === "/favicon.ico")) {
    const brandIcon = resolveBrandIconFile();
    const contentType = /\.ico$/i.test(brandIcon) ? "image/x-icon" : "image/png";
    if (serveLocalFile(res, brandIcon, contentType)) return;
  }
  if (req.method === "GET" && url.pathname === "/") return sendHtml(res, downloadLandingPage(origin));
  if (req.method === "GET" && url.pathname === "/admin") return sendHtml(res, adminPage());
  if (req.method === "GET" && url.pathname === "/download") return sendHtml(res, downloadLandingPage(origin));
  if (req.method === "GET" && url.pathname === "/health") return sendJson(res, 200, { success: true, service: "baiqiu-license-server" });
  if (req.method === "GET" && url.pathname === "/latest.json") {
    const latest = publicManifest();
    res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate");
    if (!latest?.version) return sendJson(res, 500, { message: "latest.json is missing or invalid" });
    return sendJson(res, 200, latest);
  }
  if (req.method === "POST" && (url.pathname === "/api/license/activate" || url.pathname === "/api/redeem")) return handleActivate(req, res);
  if (req.method === "GET" && url.pathname === "/api/membership/status") return handleMembershipStatus(req, res, url);
  ...
  if (req.method === "GET" && (url.pathname === "/manifest.json" || url.pathname === "/baiqiu-update.json")) {
    const published = publicManifest();
    if (!published) return sendJson(res, 500, { message: "latest.json is missing or invalid" });
    return sendJson(res, 200, {
      name: "Baiqiu AI",
      ...published,
      latestVersion: published.version,
      packageUrl: published.downloadUrl,
      checksum: published.sha256,
      fileSize: published.size,
      notes: published.releaseNotes ? [published.releaseNotes] : []
    });
    /* c8 ignore next 14 */
    ...
    return sendJson(res, 200, {
      ...
      installerUrl: downloadRelease?.installerFile ? `${origin}/download/${encodeURIComponent(downloadRelease.installerFile)}` : "",
  ...
  if (req.method === "GET" && url.pathname === "/update.json") {
    ...
    return sendJson(res, 200, {
      ...
      fileSize: published.size,
      forceUpdate: false,
      changelog: published.releaseNotes
    });
    /* c8 ignore next 10 */
    const release = latestUpdateRelease("stable");
    const downloadRelease = latestRelease("stable");
  ...
  if (req.method === "GET" && url.pathname === "/api/update/check") {
    ...
    const published = publicManifest();
    if (!published) return sendJson(res, 500, { hasUpdate: false, latestVersion: version, message: "latest.json is missing or invalid" });
    const publicHasUpdate = compareVersion(published.version, version) > 0;
    return sendJson(res, 200, {
      hasUpdate: publicHasUpdate,
      latestVersion: published.version,
      releaseNotes: published.releaseNotes,
      downloadUrl: publicHasUpdate ? published.downloadUrl : "",
      fileSize: published.size,
      checksum: published.sha256,
  ...
  if (req.method === "GET" && url.pathname === "/api/update/download") {
    const published = publicManifest();
    if (!published) return sendJson(res, 500, { message: "latest.json is missing or invalid" });
    res.writeHead(302, { Location: published.downloadUrl, "Cache-Control": "no-store" });
    res.end();
    return;
  }
  ...
  const directFileMatch = url.pathname.match(/^\/download\/([^/]+)$/i);
  if (canDownload && directFileMatch) {
    const fileName = decodeURIComponent(directFileMatch[1]);
    if (!fileName || fileName.includes("..") || fileName.includes("/") || fileName.includes("\\")) {
      return sendJson(res, 400, { success: false, message: "非法文件名" });
    }
    const file = path.join(RELEASES_DIR, fileName);
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
      return sendJson(res, 404, { success: false, message: "文件不存在" });
    }
    const contentType = /\.exe$/i.test(fileName)
    ...
    return sendFileWithRange(req, res, file, contentType, fileName);
  }
