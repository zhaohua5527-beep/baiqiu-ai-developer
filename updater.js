...
class Updater {
  ...
  async applyUpdate(zipFilePath, options = {}) {
    ...
    const script = [
      ...
      "  if ([string]$releaseManifest.version -ne $targetVersion) { throw \"Package version mismatch: expected $targetVersion, found $($releaseManifest.version)\" }",
      "  $packageExe = Join-Path $sourcePath 'BaiqiuAI.exe'",
      "  if (!(Test-Path -LiteralPath $packageExe -PathType Leaf)) { throw 'Update package missing BaiqiuAI.exe.' }",
      "  if (!(Test-Path -LiteralPath (Join-Path $sourcePath 'resources\\app\\main.js') -PathType Leaf)) { throw 'Update package missing resources/app/main.js.' }",
      ...
      "  if ($packageExeVersion -and $packageExeVersion -ne $targetVersion) { throw \"Executable version mismatch: expected $targetVersion, found $packageExeVersion\" }",
      "  $packageVersion = Get-AppVersion $sourcePath",
      "  if (!$packageVersion) { throw 'Update package is missing version metadata.' }",
      "  if ($packageVersion -ne $targetVersion) { throw \"Package version mismatch: expected $targetVersion, found $packageVersion\" }",
      ...
      "  if ($installedVersion -ne $targetVersion) { throw \"Installed version mismatch: expected $targetVersion, found $installedVersion\" }",
      "  $installedExeVersion = Get-ExecutableVersion $exe",
      "  if ($installedExeVersion -and $installedExeVersion -ne $targetVersion) { throw \"Installed executable version mismatch: expected $targetVersion, found $installedExeVersion\" }",
      "  Write-VersionLock",
      ...
      "  if ($oldVersion -and $restoredVersion -ne $oldVersion) { $message = $message + \"; rollback version mismatch: expected $oldVersion, found $restoredVersion\" }",
