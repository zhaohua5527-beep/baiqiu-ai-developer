...
const { DevelopmentPlanner } = require("./development-planner");
const { ContentPlanner } = require("./content-planner");
...
class PlannerAgent {
  constructor({ capabilityCenter = null, experienceCenter = null, strategyOptimizer = null, knowledgeRetriever = null, reasoningEngine = null, metaLearningCenter = null, reflectionMemory = null, autonomousPlanner = null, selfImprovementEngine = null, learningOrchestrator = null, knowledgeEvolutionNetwork = null, stateManager = null, eventBus = null, experienceStore = null, strategyEngine = null, decisionEngine = null, agentManager = null, teamPlanner = null, developmentPlanner = null, contentPlanner = null } = {}) {
    ...
    this.developmentPlanner = developmentPlanner || new DevelopmentPlanner();
    this.contentPlanner = contentPlanner || new ContentPlanner();
  ...
  createPlan(intentAnalysis, context = {}) {
    ...
    const add = (task) => {
      ...
      tasks.push({
        ...
        capability: advisedTask.capability || "",
        requiredPermission: advisedTask.requiredPermission || "",
    ...
    const understanding = context.understanding
      || context.runtimeContext?.conversationUnderstanding
      || context.supervision?.understanding
      || null;
    const decision = understanding?.executionMetadata || understanding;
    const isAuthorizedContentArtifact = decision?.classification === "development_task"
      && understanding?.domain === "content"
      && decision?.responseMode === "execute"
      && decision?.routing === "task_brain";
    if (isAuthorizedContentArtifact) {
      const contentPlan = this.contentPlanner.createPlan({
        taskGoal: context.taskBrain?.task_goal || context.taskBrain?.original_goal || understanding?.taskGoal || sourceText,
        permissions: decision.permissions || {},
        tools: Array.isArray(context.availableTools) ? context.availableTools : []
      });
      for (const step of contentPlan.steps) add(step);
      return this.finish(
        { ...intentAnalysis, primaryIntent: "content_task" },
        { ...context, plannerStartedAt, contentPlan },
        tasks,
        true,
        knowledgeHints,
        reasoningResult,
    ...
    const isAuthorizedDevelopment = decision?.classification === "development_task"
      && decision?.responseMode === "execute"
      && decision?.routing === "task_brain";
    if (isAuthorizedDevelopment) {
      const developmentPlan = this.developmentPlanner.createPlan({
        taskGoal: context.taskBrain?.task_goal || context.taskBrain?.original_goal || understanding?.taskGoal || sourceText,
        permissions: decision.permissions || {},
        tools: Array.isArray(context.availableTools) ? context.availableTools : [],
        appRoot: context.appRoot || ""
      });
      for (const step of developmentPlan.steps) add(step);
      return this.finish(
        { ...intentAnalysis, primaryIntent: "development_task" },
        { ...context, plannerStartedAt, developmentPlan },
        tasks,
        true,
        knowledgeHints,
        reasoningResult,
  ...
  finish(intentAnalysis, context, tasks, isMultiStep, knowledgeHints = null, reasoningResult = null, metaHints = null, reflectionHints = null, autonomousPlan = null, improvementHints = null, learningHints = null, evolutionHints = null, experienceMemoryHints = [], strategyResult = null, strategyDecision = null, agentTeam = null) {
    ...
    if (context.developmentPlan) {
      plan.planType = context.developmentPlan.planType;
      plan.requiredCapabilities = [...context.developmentPlan.requiredCapabilities];
      plan.missingCapabilities = [...context.developmentPlan.missingCapabilities];
      plan.workerRequirement = context.developmentPlan.workerRequirement;
      plan.developmentStatus = context.developmentPlan.status;
    }
    if (context.contentPlan) {
      plan.planType = context.contentPlan.planType;
      plan.contentType = context.contentPlan.contentType;
      plan.requiredCapabilities = [...context.contentPlan.requiredCapabilities];
      plan.missingCapabilities = [...context.contentPlan.missingCapabilities];
      plan.contentStatus = context.contentPlan.status;
      plan.resumeOriginalGoal = context.contentPlan.resumeOriginalGoal;
    }
    this.eventBus.publish(AGENT_EVENTS.PLAN_CREATED, {
      sessionId: context.sessionId || context.traceId || intentAnalysis.context?.sessionId || "global",
