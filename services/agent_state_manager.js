...
class AgentStateManager {
  ...
  consumeEvent(event = {}) {
    ...
    const { taskContext: _taskContext, ...safeData } = data;
    if (event.type === AGENT_EVENTS.TOOL_SELECTED) return this.recordToolSelection(event.sessionId, safeData);
    if (event.type === AGENT_EVENTS.TOOL_RESULT) return this.recordExecution(event.sessionId, safeData);
    if (event.type === AGENT_EVENTS.VERIFICATION_DONE) return this.recordVerification(event.sessionId, safeData.verification || safeData);
    if (event.type === AGENT_EVENTS.MEMORY_UPDATED) return this.recordLearning(event.sessionId, safeData);
    if (event.type === AGENT_EVENTS.PLAN_CREATED) return this.transition(event.sessionId, state, { ...safeData, plan: safeData.plan || safeData });
    return this.transition(event.sessionId, state, safeData);
  }
  ...
  recordToolSelection(sessionId = "global", tool = {}) {
    ...
    const record = {
      ...
      logicalTool: tool.logicalTool || "",
      intent: tool.intent || tool.userIntent || "",
      taskId: tool.taskId || ""
    };
    if (!context.tools.some((item) => (item.toolId || item.id || item.logicalTool) === (record.toolId || record.logicalTool))) {
      context.tools.push(record);
    }
    return this.transition(sessionId, AGENT_STATES.SELECTING_TOOL, {
      tools: context.tools,
  ...
  recordLearning(sessionId = "global", update = {}) {
    const context = this.getTaskContext(sessionId);
    context.memoryUpdates.push({
      type: update.type || update.memoryType || "memory_update",
      source: update.source || "",
      key: update.key || update.memoryId || "",
      status: update.status || "updated",
      timestamp: Number(update.timestamp || now())
    });
