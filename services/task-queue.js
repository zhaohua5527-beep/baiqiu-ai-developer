const VALID_STATES = new Set(["waiting", "running", "success", "failed", "cancelled", "verifying", "retry", "timeout", "interrupted"]);
const TERMINAL_STATES = new Set(["success", "failed", "timeout", "cancelled", "interrupted"]);
...
class TaskQueue {
  ...
  reconcileInterrupted({ activeTaskIds = [], reason = "应用启动时未找到仍在运行的任务上下文" } = {}) {
    ...
    const repaired = [];
    for (const task of db.queue) {
      if (!task?.id || TERMINAL_STATES.has(task.status) || active.has(task.id) || active.has(task.taskId)) continue;
    ...
    if (repaired.length) this.saveDb(db);
    return repaired;
  }
  ...
  get(taskId = "", sessionId = "") {
    ...
    if (!id) return null;
    return this.list(sessionId).find((item) => item.id === id || item.taskId === id) || null;
  }
