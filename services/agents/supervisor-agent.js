...
class SupervisorAgent {
  ...
  analyze(input = {}) {
    ...
    const understanding = input.understanding && typeof input.understanding === "object" ? input.understanding : null;
    ...
    const intentAnalysis = understanding ? {
      text: understanding.context?.normalizedInput || understanding.goal || "",
      clauses: [...(understanding.context?.clauses || [])],
      primaryIntent: understanding.context?.domainIntent || "general.chat",
      intents: [{ intent: understanding.context?.domainIntent || "general.chat", clause: understanding.goal || "" }],
      responseMode: understanding.context?.responseMode || "conversation",
      isMultiStep: Number(understanding.context?.taskSpec?.plan?.length || 0) > 1,
      confidence: 1,
      conversationUnderstanding: understanding
    } : this.intentAgent?.analyze?.(userMessage, input.context || {}) || {
      text: userMessage,
    ...
    const needPlan = understanding ? understanding.shouldCreateTask === true : intent !== "general.chat";
