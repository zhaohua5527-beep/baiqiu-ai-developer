...
function cleanError(error) {
  ...
  return first
    ...
    .replace(/\b(?:task|result|trace)\s*id\s*[:=]?\s*[a-z0-9:._-]+/gi, "")
    .replace(/\bverifier\b\s*[:=]?\s*/gi, "验证")
...
class ReplyBuilder {
  ...
  build({ taskResult = {}, verification = null, context = {} } = {}) {
    if (this.developerMode && wantsTechnicalLog(context)) return this.buildTechnical({ taskResult, verification });
