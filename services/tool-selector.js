const INTENT_TOOL_CATALOG = Object.freeze({
  ...
  "agent.dispatch": [{ id: "create_task", logicalTool: "worker_task_dispatch", category: "agent" }],
  "agent.task.status": [{ id: "query_task_status", logicalTool: "worker_task_status", category: "agent" }],
  "agent.deliverable": [{ id: "collect_deliverable", logicalTool: "worker_deliverable", category: "agent" }],
...
class ToolSelector {
  ...
  approveToolCall({ toolId, params = {}, intent = "", context = {}, availableTools = [] } = {}) {
    ...
    const result = {
      selectedTools: [{ ...tool, params }],
      reason: `工具 ${toolId} 已通过 ToolSelector 审核。`,
      confidence: Math.max(selected.confidence, 0.66),
      intent: normalizedIntent,
      approved: true
    };
    this.trace("INFO", "[ToolSelector]", {
    ...
    metric("ToolSelector.approve", { duration: Date.now() - startedAt, success: true, hit: true });
    return result;
  }
