  ...
  async execute({ sessionId, message, planObject, contextPatch = {}, signal = null } = {}) {
    ...
    const queueResult = await this.taskQueue.executePlan({
      ...
      executeTool: ({ toolId, args, task, context }) => {
        ...
        const brainTaskId = context.taskBrain?.task_id || "";
        if (brainTaskId) this.taskBrain?.beginStep?.(brainTaskId, task.title || task.name || toolId);
        ...
        return this.toolExecutionService.execute({
          ...
          context: {
            ...context,
            taskBrain: brainTaskId ? this.taskBrain?.executionContext?.(brainTaskId) || context.taskBrain : context.taskBrain,
            agentIntent: task.intent,
            logicalTool: toolId,
            userMessage: context.taskMessage || message
          }
        }).then((execution) => {
          if (brainTaskId) this.taskBrain?.recordStep?.(brainTaskId, task.title || task.name || toolId, Boolean(execution?.success), execution?.error || execution?.response?.error || "");
          ...
          if (brainTaskId) this.taskBrain?.recordStep?.(brainTaskId, task.title || task.name || toolId, false, error?.message || String(error));
          this.eventBus.publish(AGENT_EVENTS.TOOL_RESULT, {
            sessionId,
            traceId: context.traceId || "",
            taskId: task.id || task.taskId || "",
