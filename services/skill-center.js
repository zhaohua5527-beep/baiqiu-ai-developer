...
class SkillCenter {
  ...
  load() {
    ...
    this.store = {
      ...defaultStore(),
      ...raw,
      skills: Array.isArray(raw.skills) ? raw.skills.map((skill) => ({
        ...skill,
        status: String(skill.status || "UNVERIFIED").toUpperCase() === "INSTALLED"
          ? "UNVERIFIED"
          : String(skill.status || "UNVERIFIED").toUpperCase()
      })) : [],
  ...
  listInstalledSkills() {
    return this.listSkills().filter((skill) => skill.status === "READY");
  }
  ...
  createSkillRecord({ name, description = "", tools = [], source = "manual", status = "REGISTERED", metadata = {} } = {}) {
    const record = this.upsertSkill({ id: safeId(name), name, description, tools, source, status, metadata });
    if (record.status === "READY") this.rememberInstalledSkill(record);
    return {
  ...
  verifySkill(id) {
    ...
    const pipelineChecks = skill?.metadata?.verification?.checks || [];
    const checks = [
      { name: "record_exists", passed: Boolean(skill) },
      { name: "not_deleted", passed: Boolean(skill && skill.status !== "deleted") },
      { name: "ready_status", passed: Boolean(skill && skill.status === "READY") },
      { name: "runtime_verification", passed: Boolean(pipelineChecks.length && pipelineChecks.every((check) => check.passed)) }
    ];
    return {
