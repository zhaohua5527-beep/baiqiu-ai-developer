...
function defaultAgents() {
  return [
    ...
    {
      agentId: "developer-worker",
      name: "DeveloperWorker",
      role: "developer",
      skills: ["development", "source_inspection", "testing"],
      capabilities: ["code_search", "code_read", "code_modify", "test_execute"],
      successRate: 1,
      taskHistory: [],
      availability: "available",
      confidence: 0.9
    ...
    {
      agentId: "writing-worker",
      name: "WritingWorker",
      role: "writer",
      skills: ["writing", "research", "document"],
      capabilities: ["text_generation", "writing", "research", "document_generation"],
      successRate: 1,
      taskHistory: [],
      availability: "available",
      confidence: 0.9
...
class AgentRegistry {
  ...
  ensureStore() {
    ...
    if (!fs.existsSync(this.filePath)) {
      this.write({ agents: defaultAgents() });
      return;
    }
    const current = this.loadExisting();
    let changed = false;
    for (const defaultAgent of defaultAgents()) {
      const existing = current.agents.find((agent) => agent.agentId === defaultAgent.agentId);
      if (!existing) {
        current.agents.push(defaultAgent);
        changed = true;
        continue;
      }
      const mergedCapabilities = [...new Set([...(existing.capabilities || []), ...defaultAgent.capabilities])];
      if (mergedCapabilities.length !== (existing.capabilities || []).length) {
