...
class AgentEventBus {
  ...
  publish(type, payload = {}) {
    ...
    if (this.consumedEvents.has(consistencyKey)) {
      const duplicateDecision = this.governance.check(event);
      if (duplicateDecision.allowed === false) {
        const blockedDuplicate = { ...event, duplicate: true, blocked: true, governance: duplicateDecision };
        this.dispatch(blockedDuplicate);
