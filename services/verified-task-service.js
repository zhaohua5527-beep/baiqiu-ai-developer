...
class VerifiedTaskService {
  ...
  async openBrowser({ path: rawPath = "", url = "", target = "", sessionId = "", source = "", signal = null, internalApp = false } = {}) {
    this.ensureActive(signal);
    ...
    if (/^https?:\/\//i.test(value)) {
      ...
      const opened = await this.deps.openInternalBrowser(value, { sessionId, source });
      this.log("browser_open", "open_web", "success", { url: value, browser: "black-ball", sessionId, source });
      return opened;
    }
