...
function extractFileReferences(message = "") {
  ...
  const quoted = text.match(/["“'‘]([^"”'’]+\.(?:xlsx|xls|csv|png|jpe?g|webp|bmp|gif|txt|md|json|html?|docx|pdf|pptx|zip))["”'’]/gi) || [];
  ...
  const pattern = /(?:[A-Za-z]:[\\/][^\r\n"'<>|]+?|[^\s\r\n"'<>|，。；;：:]+?)\.(?:xlsx|xls|csv|png|jpe?g|webp|bmp|gif|txt|md|json|html?|docx|pdf|pptx|zip)(?![A-Za-z0-9])/gi;
  let match;
...
function isAnalysisIntent(message = "") {
  ...
  return /(分析|读取|读一下|查看|看看|识别|总结|解释|这个文件|这份文件|这个表格|这张图|图片|照片|图像|表格|excel|xlsx|csv|json|文档|文件|项目包|压缩包|word|pdf|ppt)/i.test(text);
}
...
function textContentFromFile(file, mimeType) {
  if (!/\.(txt|md|json|csv|html?)$/i.test(file)) return "";
  try {
...
function analysisHint({ targets = [], usedLastTarget = false } = {}) {
  ...
  return [
    ...
    "如果是 Word、PDF、PPT、文本或 JSON，请提炼关键信息、风险点和可执行建议。",
    "如果是 ZIP 项目包，请先识别技术栈、入口文件和目录风险，再给出检查建议。"
  ].join("\n");
}
