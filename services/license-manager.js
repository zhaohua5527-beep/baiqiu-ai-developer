...
class LicenseManager {
  constructor(options = {}) {
    ...
    this.membershipPath = options.membershipPath || path.join(os.homedir(), "AppData", "Roaming", "Baiqiu AI", "data", "membership", "membership.json");
  ...
  getStatus() {
    ...
    const restoredFromMirror = this._restoreMembershipMirror(db);
    ...
    if (license.unlocked && !local.ok) {
      ...
      changed = !license.unlocked
        || license.locked
        || license.state !== "activated"
        || Boolean(license.lockReason)
        || !license.userId
        || changed;
      ...
      license.state = "activated";
      license.lockReason = "";
      license.userId ||= this._membershipUserId(license.customer || {}, this._decryptField(license.activatedCode || ""));
    }

    if (!license.unlocked && !licenseInvalid) {
    ...
    if (license.unlocked && local.ok) this._writeMembershipMirror(license);
    return this._publicStatus(license, restoredFromMirror ? "membership_mirror" : "local_database");
  }

  async restoreMembership() {
    const local = this.getStatus();
    if (local.unlocked) return { ...local, restored: "local" };
    const cloud = await this._requestJson("GET", `/api/membership/status?deviceId=${encodeURIComponent(this.deviceId)}`);
    if (!cloud?.success || !cloud?.membership) return { ...local, restored: false, cloudMessage: cloud?.message || "云端没有可恢复的会员状态" };
    const membership = cloud.membership;
    const code = String(membership.activationCode || membership.code || "").trim().toUpperCase();
    const expiresAt = membership.expireTime || membership.expiresAt || "2099-12-31T23:59:59Z";
    if (!code || !this._verifyServerSignature(code, this.deviceId, expiresAt, membership.signature || "")) {
      return { ...local, restored: false, cloudMessage: "云端会员签名校验失败" };
    }
    this._activateLicense(code, expiresAt, membership.signature, "cloud-restore", membership.customer || {}, {
      plan: membership.membershipLevel || membership.plan || "",
      planName: membership.planName || "会员",
      lifetime: Boolean(membership.lifetime),
      userId: membership.userId || "",
      activationHistory: membership.activationHistory || []
    });
    return { ...this.getStatus(), restored: "cloud" };
  }
  ...
  async verifyCode(code, customer = {}) {
    ...
    this._activateLicense(normalized, expiresAt, serverSignature, online.endpoint || "online", activationCustomer, {
      ...
      userId: license.userId || online.userId || "",
      activationHistory: license.activationHistory || online.activationHistory || []
    });
  ...
  async createPaidOrder(payload = {}) {
    return this._requestJson("POST", "/api/order/create", {
      ...
      name: payload.name || payload.customer || "",
      phone: payload.phone || "",
  ...
  _activateLicense(code, expiresAt, serverSignature, activationMode, customer = {}, membership = {}) {
    ...
    license.userId = String(membership.userId || license.userId || this._membershipUserId(customer, code));
    license.createdAt = Number(license.createdAt || Date.now());
    const history = Array.isArray(membership.activationHistory) ? membership.activationHistory : license.activationHistory;
    license.activationHistory = Array.isArray(history) ? history.slice(-20) : [];
    license.activationHistory.push({ deviceId: this.deviceId, mode: activationMode || "online", activatedAt: Date.now() });
    license.activationHistory = license.activationHistory.slice(-20);
    license.trialStartAt = null;
    license.trialLastTickAt = null;
    license.trialUsedSeconds = 0;
    this._signLicense(license);
    this._writeDb(db);
    this._writeMembershipMirror(license);
  ...
  setSecurityBlock(message = "程序安全校验未通过") {
    const db = this._readDb();
    const license = this._ensureLicense(db);
    license.securityBlocked = true;
    license.securityBlockReason = String(message || "程序安全校验未通过");
    license.securityBlockedAt = Date.now();
    license.securitySignature = this._sign({ blocked: true, reason: license.securityBlockReason, at: license.securityBlockedAt });
    this._writeDb(db);
    return this._publicStatus(license, "local_database");
  }

  clearSecurityBlock() {
    const db = this._readDb();
    const license = this._ensureLicense(db);
    if (!license.securityBlocked && !license.securityBlockReason) return this._publicStatus(license, "local_database");
    license.securityBlocked = false;
    license.securityBlockReason = "";
    license.securityBlockedAt = null;
    license.securitySignature = "";
    this._writeDb(db);
    return this._publicStatus(license, "local_database");
  }

  lock(message = "程序安全校验未通过") {
    return this.setSecurityBlock(message);
  }
  ...
  _publicStatus(license, source = "local_database") {
    ...
    return {
      ...
      activationStatus: license.unlocked ? "ACTIVE" : license.locked ? "INACTIVE" : "TRIAL",
      unlocked: Boolean(license.unlocked),
      locked: Boolean(license.locked) && !license.unlocked,
      securityBlocked: Boolean(license.securityBlocked),
      securityMessage: license.securityBlockReason || "",
      source,
      ...
      deviceBinding: this.deviceId,
      activationHistory: Array.isArray(license.activationHistory) ? license.activationHistory.slice(-20) : [],
  ...
  _membershipUserId(customer = {}, code = "") {
    const identity = String(customer.phone || customer.name || code || this.deviceId).trim().toLowerCase();
    return `user-${crypto.createHash("sha256").update(identity).digest("hex").slice(0, 20)}`;
  }

  _membershipMirrorPayload(license) {
    return {
      schemaVersion: 1,
      userId: license.userId || this._membershipUserId(license.customer || {}, this._decryptField(license.activatedCode || "")),
      membershipLevel: license.plan || (license.lifetime ? "lifetime" : "member"),
      expireTime: license.expiresAt || "",
      activationCode: license.activatedCode || "",
      deviceBinding: this.deviceId,
      activationHistory: Array.isArray(license.activationHistory) ? license.activationHistory.slice(-20) : [],
      createdAt: Number(license.createdAt || license.activatedAt || Date.now()),
      updatedAt: Date.now(),
      license: JSON.parse(JSON.stringify(license))
    };
  }

  _writeMembershipMirror(license) {
    try {
      const payload = this._membershipMirrorPayload(license);
      fs.mkdirSync(path.dirname(this.membershipPath), { recursive: true });
      const temp = `${this.membershipPath}.tmp-${process.pid}-${Date.now()}`;
      fs.writeFileSync(temp, JSON.stringify({ ...payload, checksum: this._sign(payload) }, null, 2), "utf8");
      fs.renameSync(temp, this.membershipPath);
    } catch (error) {
      console.warn("[License] 会员镜像保存失败:", error.message || error);
    }
  }

  _restoreMembershipMirror(db) {
    try {
      const mirror = JSON.parse(fs.readFileSync(this.membershipPath, "utf8"));
      const { checksum, ...payload } = mirror || {};
      if (!checksum || !this._verify(payload, checksum) || payload.deviceBinding !== this.deviceId) return false;
      const mirroredLicense = payload.license;
      if (!mirroredLicense || !this._verifyLicense(mirroredLicense).ok || !this.verifyLocal(mirroredLicense).ok) return false;
      const current = this._ensureLicense(db);
      if (current.unlocked && this.verifyLocal(current).ok) return false;
      db.settings.license = JSON.parse(JSON.stringify(mirroredLicense));
      this._writeDb(db);
      return true;
    } catch {
      return false;
